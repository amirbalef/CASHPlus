...
class BarDistribution(nn.Module):
    ...
    def map_to_bucket_idx(self, y):
        #print("map_to_bucket_idx", y.shape)
    ...
    def forward(self, logits, y):
        losses = [
            self._forward(logits[..., inx], y[..., inx]) for inx in range(y.shape[-1])
        ]
...
class FullSupportBarDistribution(BarDistribution):
    ...
    def forward(self, logits, y):
        losses = [self._forward(logits[...,inx], y[...,inx]) for inx in range(y.shape[-1])]
        loss = torch.mean(torch.stack(losses), dim = 0)
        return loss

...
class LCPFN(torch.nn.Module):
    def __init__(self, model_name="EMSIZE512_NLAYERS12_NBUCKETS1000", device="cpu"):
    ...
    def predict_mean(
        ...
        return torch.stack(
            [
                normalizer[1](self.model.criterion.mean(logits[..., i]))
    ...
    def predict_quantiles(
        ...
        return torch.stack([
            normalizer[1](
                torch.cat(
                    [self.model.criterion.icdf(logits[..., i], q) for q in qs], dim=1
                )
            )
            for i in range(logits.shape[-1])
        ], dim=-1,

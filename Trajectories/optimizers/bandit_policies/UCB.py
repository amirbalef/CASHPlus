        ...
        if np.any(pulled_arms == 0):
            ...
            if self.tuned:
                mean_rewards = np.asarray(
                    [
                        np.mean(self.rewards[np.asarray(self.pulled_arms) == x])
                        for x in range(self.narms)
                    ]
                )
                variance_rewards = np.asarray(
                    [
                        np.var(self.rewards[np.asarray(self.pulled_arms) == x])
                        for x in range(self.narms)
                    ]
                )
                tuned_padding = variance_rewards + 2 * (np.log(self.t)) / pulled_arms
                tuned_padding[tuned_padding < 0.25] = 0.25
                ucb_values = mean_rewards + np.sqrt(
                    np.log(self.t) / pulled_arms * tuned_padding
                )
            else:
                mean_rewards = np.asarray(
                    [
                ...
                ucb_values = mean_rewards + np.sqrt(
                    self.alpha * (np.log(self.t)) / pulled_arms
                )

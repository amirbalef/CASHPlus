...
class QoMax_SDA:
    ...
    def QoMax_duel(self, l, k):
        ...
        self.qomax[k] = QoMax(
            self.rewards[np.asarray(self.pulled_arms) == k],
            batch=self.nb_batch[k],
            n=self.nb_k[k],
            q=self.q,
        )
        # Compute leader's QoMax (on Last Block subsample)
        Y_l = self.rewards[np.asarray(self.pulled_arms) == l][
            self.nb_k[l] - self.nb_k[k] : self.nb_k[l] * self.nb_batch[k]
        ]
